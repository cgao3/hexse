...
def mnist_test_accuracy(ckpt, meta_graph):
    from tensorflow.examples.tutorials.mnist import input_data
    mnist = input_data.read_data_sets('MNIST_data', one_hot=True)
    loader=Loader(checkpoint_file=ckpt, meta_file=meta_graph,
                  op_names_to_feed=['x_inputs_node', 'y_labels_node'], op_name_to_run='accuracy_op')
    res=loader.evaluate_op_to_run(feed_data=[mnist.test.images, mnist.test.labels])
    print('result: ', res)
    loader.close()

def nine_by_nine_hex_test_accuracy(ckpt, meta, src_position_action_test_file):
    loader=Loader(checkpoint_file=ckpt, meta_file=meta, op_names_to_feed=['x_inputs_node','y_star_node'],
                  op_name_to_run='accuracy_evaluate_node')
    ...
    while True:
        ...
        res=loader.evaluate_op_to_run(feed_data=[reader.batch_positions, reader.batch_labels])
        acc += res
        step +=1
...
if __name__ == "__main__":
    ...
    nine_by_nine_hex_test_accuracy(ckpt=args.checkpoint, meta=args.meta_graph,
                                   src_position_action_test_file=args.src_test)
